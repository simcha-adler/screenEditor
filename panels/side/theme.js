...
const themeSchema = [
    // כותרת
    { type: 'title', label: 'מערכת צבעים חכמה' }, // או smallTitle לפי העדפה

    // קונטיינר לרשימת הפריטים
    {
        type: 'div',
        id: 'themeItemsList',
        // אם תרצה להוסיף קלאס ספציפי:
    ...
    // דיב לאוברליי (אם הוא עדיין נדרש בתוך הפאנל הזה)
    {
        type: 'div',
        id: 'themeEditorOverlay'
    }
];
 ...
 */
function createColorPicker(selector, prop) {
    ...
    container.innerHTML = `
        <div class="picker-tabs" style="display:flex; gap:5px; margin-bottom:5px;">
            <button class="tab-btn ui-btn ${mode === 'solid' ? 'active' : ''}" data-mode="solid">רגיל</button>
            <button class="tab-btn ui-btn ${mode === 'theme' ? 'active' : ''}" data-mode="theme">מתוך טווח</button>
        </div>
        
        <div class="mode-content solid-mode ${mode === 'solid' ? '' : 'hide'}">
            <input type="color" class="simple-color" value="#000000" data-property=${prop}}" style="width:100%;">
        </div>

        <div class="mode-content theme-mode ${mode === 'theme' ? '' : 'hide'}">
            ...
            <div class="range-slider-container ${existingLink ? '' : 'hide'}" style="position:relative; height:20px; border-radius:10px; border:1px solid #ccc; margin-top:5px;">
    ...
    container.$$('.tab-btn').whenClick((e) => {
        ...
        container.$$('.mode-content').addClass('hide');
        container.$1(`.${m}-mode`).removeClass('hide');
    ...
    themeSelect.when('change', (e) => {
        ...
        if (!themeId) {
            rangeContainer.addClass('hide');
            return;
        }

        rangeContainer.removeClass('hide');
